...
curl.exe -o %USERPROFILE%\.konspekt\konspekt.zip -L https://github.com/dadaismee/konspekt-research-pack/archive/refs/heads/main.zip
...
if not exist %appdata%\obsidian\obsidian.json (
	...
	>nul find "research_pack" %appdata%\obsidian\obsidian.json && (
		...
		powershell.exe "(Get-Content $env:APPDATA\obsidian\obsidian.json) -replace '}}}','},\"8095a1a7a15b1e3d\":{\"path\":\"C:\\Users\\test\\research_pack\",\"ts\":1739264225722,\"open\":true}},\"showReleaseNotes\":false}' | Out-File -encoding ASCII $env:APPDATA\obsidian\obsidian.json"
		powershell.exe "(Get-Content $env:APPDATA\obsidian\obsidian.json) -replace '}},','}, \"8095a1a7a15b1e3d\":{\"path\":\"C:\\Users\\test\\research_pack\",\"ts\":1739264225722,\"open\":true}},\"showReleaseNotes\":false}' | Out-File -encoding ASCII $env:APPDATA\obsidian\obsidian.json"
		powershell.exe "(Get-Content $env:APPDATA\obsidian\obsidian.json) -replace 'test',$env:USERNAME | Out-File -encoding ASCII $env:APPDATA\obsidian\obsidian.json"
	)
)

rem %USERPROFILE%\scoop\shims\obsidian-cli set-default research_pack
powershell "obsidian-cli set-default research_pack"
timeout 1
...
if not exist %userprofile%\Zotero\zotero.sqlite (
	...
	rem copy %userprofile%\.konspekt\konspekt-research-pack-main\zotero.sqlite %userprofile%\Zotero\zotero.sqlite
